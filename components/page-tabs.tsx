...
export function PageTabs({ tabs, activeId }: PageTabsProps) {
  return (
    <nav
      aria-label="Sub-tab"
      className="mb-6 flex flex-wrap gap-2 rounded-2xl bg-white/80 p-2 shadow-card ring-1 ring-primary/5 dark:bg-d-surface/70 dark:ring-d-border/30"
    >
      {tabs.map((tab) => {
        ...
        return (
          <Link
            ...
            className={`inline-flex items-center gap-2 rounded-xl px-4 py-2 text-[12.5px] font-bold transition ${
              active
